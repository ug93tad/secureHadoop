 ...
 */
public class BaselinePagerankEG extends Configured implements Tool {
	...
	static class MapStage1 extends Mapper<BytesWritable, ObjectWritable, IntWritable, Text> {
		...
		@Override
		public void map(BytesWritable key, ObjectWritable value, Context context)
				throws IOException, InterruptedException {
			byte[] iv = key.copyBytes();
			byte[] ct; 
			Object obj = value.get(); 
			String[] line; 
			if (obj instanceof EdgeCiphertext){
				ct = ((EdgeCiphertext)obj).getContent().copyBytes();
				line = new String(rand.decrypt_word_rnd(ct, iv)).split("\t");
				context.write(new IntWritable(Integer.parseInt(line[0])), new Text(line[1])); 
			}						
			else if (obj instanceof PRCiphertext){
				ct = ((PRCiphertext)obj).getContent().copyBytes();
				line = new String(rand.decrypt_word_rnd(ct, iv)).split("\t");
				context.write(new IntWritable(Integer.parseInt(line[0])), new Text("s"+line[1]));
			}
		}
	}

	static class RedStage1 extends Reducer<IntWritable, Text, BytesWritable, ObjectWritable> {
		int number_nodes = 0;
		Rand rand; 
		...
		public void reduce(IntWritable key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			for (IntWritable iw : outedges){
				...
				byte[] newCt = rand.encrypt_word_cbc(iw.get()+"\t"+normVal, newIv);
				context.write(new BytesWritable(newIv), new ObjectWritable(
						new PRCiphertextNormalized(newCt)));
			}
		}
	}
	
	static class MapStage2 extends Mapper<BytesWritable, ObjectWritable, IntWritable, Text>
    {
		Rand rand;
		@Override
		public void setup(Context context){
		...
		// Identity mapper
		@Override
		public void map (BytesWritable key, ObjectWritable value, Context context) throws IOException, InterruptedException
		{
			byte[] iv = key.copyBytes();
			byte[] ct; 
			Object obj = value.get(); 
    ...
    static class RedStage2 extends Reducer<IntWritable, Text, BytesWritable, ObjectWritable>
    {
		int number_nodes = 0;	
		Rand rand; 
		...
		public void reduce(IntWritable key, Iterable<Text> values,
			...
			ct = rand.encrypt_word_rnd(key.toString()+"\t"+next_rank, iv); 
    ...
    static class DecryptAndSortMapper extends Mapper<BytesWritable, ObjectWritable, DoubleWritable, Text>{
    	Rand rand; 
    	...
    	public void map(BytesWritable key, ObjectWritable value, Context context) throws IOException, InterruptedException{    		
    		...
    		String[] line = new String(rand.decrypt_word_rnd(
					pr.getContent().copyBytes(), iv)).split("\t"); 
			double rank = Double.parseDouble(line[1]); 
    		context.write(new DoubleWritable(rank), new Text(line[0]));     		    		
    	}
    }
	...
	private void runStage1() throws IOException, ClassNotFoundException, InterruptedException{
		...
		job.setJarByClass(BaselinePagerankEG.class); 
	...
	public static void main(String[] args) throws Exception {
		ToolRunner.run(new BaselinePagerankEG(), args);
	}
}
