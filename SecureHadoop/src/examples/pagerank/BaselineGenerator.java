 ...
 * 
 */
public class BaselineGenerator extends Configured implements Tool{
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(BaselineGenerator.class); 
	...
	public static void main(String args[]) throws Exception {
		...
		ToolRunner.run(new BaselineGenerator(), args); 
		
		Configuration conf = new Configuration(); 
		int n = new Integer(args[0]).intValue(); 
		Path path = new Path("/pr_vector");
		BaselineGenerator.gen_initial_vector(n, path, conf);
		Path path1 = new Path("/pref_vector"); 
		BaselineGenerator.gen_pref_vector(n, path1, conf);
	}
