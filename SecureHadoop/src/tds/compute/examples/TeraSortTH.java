...
import tds.encode.examples.TerasortEncryptor;
import tds.hom.Ope;
...
/**
 * Same as Tersort, but only uses simplePartitioner
 */
public class TeraSortTH extends Configured implements Tool {
  private static final Log LOG = LogFactory.getLog(TeraSortTH.class);
  ...
  static class TotalOrderPartitioner extends Partitioner<Text,Text>
    ...
    public void setConf(Configuration conf) {
      try {
        ...
        this.conf = conf;
        Path partFile = new Path(TeraEncryptedInputFormat.PARTITION_FILENAME);
  ...
  public int run(String[] args) throws Exception {
    ...
    job.setJarByClass(TeraSortTH.class);
	...
	c = job.getCounters().findCounter(TdsOptions.HEEDOOP_COUNTER.RND_TO_OPE); 
	System.out.println("Counter "+c.getDisplayName()+" : "+c.getValue()); 
	return result; 
  }
  ...
  public static void main(String[] args) throws Exception {
    int res = ToolRunner.run(new Configuration(), new TeraSortTH(), args);
    System.exit(res);
  }

	/**
	 * Reading from the encrypted file, decrypt and oput <key,val> in text
	 */
	static class SortMapper extends Mapper<Text, Text, Text, Text> {
		...
		public void setup(Context context) {
			...
			reEncryptor = new Ope(); 
			reEncryptor.initPrivateParameters(Utils.getOpeParams(conf)); 
		}

		@Override
		public void map(Text key, Text value, Context context)
				throws IOException, InterruptedException {			 			
			byte[] pt = crypto.decryptRaw(value.copyBytes()); 
			context.getCounter(TdsOptions.HEEDOOP_COUNTER.RND).increment(1); 
			byte[] k = new byte[TerasortEncryptor.PT_KEY_LENGTH];
			byte[] v = new byte[TerasortEncryptor.PT_VALUE_LENGTH];
			
			System.arraycopy(pt, 0, k, 0, TerasortEncryptor.PT_KEY_LENGTH);
			System.arraycopy(pt, TerasortEncryptor.PT_KEY_LENGTH, v, 0,
					TerasortEncryptor.PT_VALUE_LENGTH);
			context.write(new Text(reEncryptor.encryptRaw(k)), new Text(crypto.encryptRaw(v)));
			context.getCounter(TdsOptions.HEEDOOP_COUNTER.RND_TO_OPE).increment(1); 
		}
	}
	 ...
	 * identity reducer
	 */
	static class SortReducer extends Reducer<Text, Text, Text, Text> {
		
		//similar to BaselineSortEncryptor
		@Override
