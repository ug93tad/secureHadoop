 ...
 * 
 */
public class PagerankGenerator extends Configured implements Tool{
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(PagerankGenerator.class); 
	...
	/**
	 * args: n <original edge file> <new edge file> <prvector> <prefvector>
	 * @throws Exception 
	 */
	public static void main(String args[]) throws Exception {
		//convert the edge file
		ToolRunner.run(new PagerankGenerator(), args); 
		
		Configuration conf = new Configuration(); 
		int n = new Integer(args[0]).intValue(); 
		Path path = new Path(args[3]);
		PagerankGenerator.gen_initial_vector(n, path, conf);
		Path path1 = new Path(args[4]); 
		PagerankGenerator.gen_pref_vector(n, path1, conf);
	}
