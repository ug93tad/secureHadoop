...
VERIFICATION=/HiBench/Pagerank/Output/pr_check_th
JAR=../../lib/secureHadoop.jar
JOB=tds.compute.examples.PagerankTH
CONF=mapred.xml
FILES=../../c++/libhryto.so
...
hadoop jar $JAR $JOB -conf $CONF -files $FILES -D mapred.child.env=$ENV -libjars $LIBJARS $PAGES  $REDUCERS $ITERATIONS $EDGE_IN $PR_IN $PREF_IN $PR_OUT $VERIFICATION
