...
void pad(const unsigned char* in, int len, unsigned char ** out, int *newLen){

    if (len%AES_BLOCK_SIZE == 0) {
        *newLen = len+AES_BLOCK_SIZE;
    } else {
        *newLen = (len / AES_BLOCK_SIZE + 1) * AES_BLOCK_SIZE;
    }

    *out = (unsigned char*)malloc(*newLen); 
    memcpy(*out,in,len); 
    (*out)[len] = 1; 
...
void unpad(const unsigned char* in, int len, unsigned char ** out, int *newLen){
	len--; 
