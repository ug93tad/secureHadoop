                    ...
                    string & wordKey)
{
    ...
    //L_i and R_i
    int nBlocks = ciph.length()/AES_BLOCK_SIZE; //multiple blocks
    int idx =0; 
    for (int i=0; i<nBlocks; i++){
		string L_i = ciph.substr(idx,SWPr);
		idx+=SWPr;  
	    //wordKey: k_i = PRP_{key}(E[W_i])

       	wordKey = wordKey + PRP(key, ciph).substr(0,AES_BLOCK_SIZE);
    }
}
...
string SWP::encrypt(const string & key, const string & word){
	return SWPencrypt(key, word, 0); 
}
...
SWP::SWPencrypt(const string & key, string word, unsigned int index)
{
    ...
    int nBlocks = ciph.length()/AES_BLOCK_SIZE; 
    int subWk = wordKey.length()/nBlocks; 
 
    string fu; 
    for (int i=0; i<nBlocks; i++){
	    //S_i
	    string salt = random(SWPr);
	    //F_{k_i} (S_i)
            string tmp = PRP(wordKey.substr(i*subWk,subWk), salt);
	    fu = fu+salt+tmp.substr(SWPr,SWPm);
    }
    return bytewise_xor(ciph, fu);
...
SWP::SWPsearch(const Token & token, const string & ciph)
{
    ...
    if (ciph.length() != token.ciph.length())
	return false; 
    ...
    int nBlocks = ciph.length()/AES_BLOCK_SIZE; 
    int wkLen = token.wordKey.length()/nBlocks; 

    for (int i=0; i<nBlocks; i++){
 	   //remains salt, PRP(wordkey, salt)
	    string salt = ciph2.substr(i*AES_BLOCK_SIZE, SWPr);
	    string funcpart = ciph2.substr(i*AES_BLOCK_SIZE+SWPr, SWPm);

	    string func = PRP(token.wordKey.substr(wkLen*i,wkLen), salt).substr(SWPr,SWPm);
...
bool SWP::match(const Token & token, const std::string & ciph){
	return SWPsearch(token,ciph); 
}
