...
class SWP {
 public:
    ...
    static std::string encrypt(const std::string & key, const std::string & word); 
    ...
    static bool match(const Token & token, const std::string & ciph); 

    static const bool canDecrypt = (SWPCiphSize % AES_BLOCK_SIZE == 0);

    static std::string fixedKey; 
    /** PRP **/
